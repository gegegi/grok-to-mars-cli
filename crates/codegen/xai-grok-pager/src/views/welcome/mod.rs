...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Grok To Mars** VERSION+channel (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | channel (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **Grok To Mars**  VERSION (left-aligned).
    HeroInline,
}
...
pub(super) fn render_version_badge(
    ...
    match &mode {
        VersionBadgeMode::Full { .. } => {
            spans.push(Span::styled(
                format!("{}  ", xai_grok_version::DISPLAY_NAME),
                Style::default()
                    .fg(theme.text_primary)
                    .add_modifier(Modifier::BOLD),
            ));
    ...
    let mut result = match params.auth_state {
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "Grok To Mars is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
/// Here the message is a multi-line block showing the workspace path and the warning that Grok To Mars may run or modify contents in this directory.
...
mod tests {
    ...
    fn version_badge_carries_no_release_label() {
        ...
        assert!(
            full.contains(xai_grok_version::DISPLAY_NAME),
            "full badge: {full:?}"
        );
        assert!(
            inline.contains(xai_grok_version::DISPLAY_NAME),
            "inline badge: {inline:?}"
        );
